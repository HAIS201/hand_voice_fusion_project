...
손 제스처는 방향과 위치 같은 공간적 정보를 표현하는 데 적합하며, 음성은 “공격”, “방어”와 같은 고수준 명령을 빠르게 전달하는 데 적합하다. 따라서 두 모달리티를 함께 사용하면 단일 입력 방식보다 더 자연스럽고 몰입감 있는 상호작용을 제공할 수 있다.
...
최종 Unity 데모에서는 다음과 같은 기능을 구현하였다.

* 제스처 명령에 따른 캐릭터 이동
* 음성 명령에 따른 공격 및 방어 행동 처리
* Animator Controller를 이용한 캐릭터 상태 전환
* 카메라 추적 기능을 통한 실시간 데모 화면 구성
* UDP 명령 처리 방식 개선을 통한 반복 실행 문제 완화

초기 구현에서는 캐릭터 상태를 단순한 색상 변화로 표현하였으나, 최종 구현에서는 캐릭터의 이동 및 행동 상태가 보다 직관적으로 나타나도록 개선하였다. 이를 통해 사용자가 손 제스처와 음성 명령으로 캐릭터를 제어하는 과정을 실시간으로 확인할 수 있도록 하였다.
...
※ 대용량 특징 파일(`.npy`)과 모델 가중치 파일(`.pth`)은 GitHub 업로드 용량 및 저장소 관리를 위해 포함하지 않았다.
...
Python 스크립트가 UDP 명령을 전송하면 Unity 캐릭터가 해당 명령에 따라 이동 및 행동을 수행한다.
...
또한 Unity 데모를 통해 인식 결과가 실제 캐릭터 이동 및 행동으로 연결되는 과정을 구현함으로써, 멀티모달 HCI 시스템의 실제 적용 가능성을 확인하였다.
