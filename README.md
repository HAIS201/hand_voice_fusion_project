손 제스처와 음성 인식을 활용한 멀티모달 실시간 제어 시스템

📌 프로젝트 소개
본 프로젝트는 손 제스처와 음성 명령을 함께 활용하는 멀티모달 실시간 제어 시스템을 설계하고 구현한 연구 프로젝트이다. 게임, VR, 교육용 시뮬레이션과 같은 환경에서는 키보드와 마우스를 벗어나 보다 자연스럽고 직관적인 인터페이스가 요구된다. 이에 따라 본 프로젝트에서는 손 제스처와 음성이라는 서로 다른 입력 모달리티를 결합하여 캐릭터를 제어하는 방식을 구현하였다.

본 시스템에서 손 제스처는 캐릭터의 이동 명령을 담당하고, 음성은 공격 및 방어와 같은 행동 명령을 담당한다. 손 제스처는 방향과 위치 같은 공간적 정보를 표현하는 데 적합하며, 음성은 “공격”, “방어”와 같은 고수준 명령을 빠르게 전달하는 데 적합하다. 따라서 두 모달리티를 함께 사용하면 단일 입력 방식보다 더 자연스럽고 직관적인 상호작용을 제공할 수 있다.

본 프로젝트의 주요 목표는 다음과 같다.

- 웹캠과 마이크를 이용한 손 제스처 및 음성 데이터 직접 수집
- MediaPipe Hands와 Mel-spectrogram 기반 특징 추출 파이프라인 구현
- GRU 기반 Early Fusion 및 Late Fusion 모델 학습
- 동일한 데이터셋에서 두 융합 방식의 성능 비교
- Python 인식 시스템과 Unity를 UDP로 연동한 실시간 제어 데모 구현
...
전체 메타데이터는 labels.csv로 관리하였고, 학습·검증·테스트 분할은 splits/train.csv, splits/val.csv, splits/test.csv로 구성하였다.
...
환경별 결과에서는 BrightQuiet 환경에서 가장 높은 성능이 나타났고, DimFan 환경에서 가장 낮은 성능이 나타났다. 이는 저조도와 배경 소음이 동시에 존재하는 환경이 멀티모달 인식 시스템에 상대적으로 더 어려운 조건일 수 있음을 보여준다.

🎮 Unity 실시간 데모
본 프로젝트에서는 Python 기반 인식 시스템과 Unity를 UDP 통신으로 연동하여 실시간 캐릭터 제어 데모를 구성하였다.
...
Unity 구현 내용
Unity에서는 UDPReceiver.cs가 Python에서 전송한 UDP 문자열을 수신하고, CommandController.cs가 이를 해석하여 캐릭터 제어에 반영한다.

데모에서는 캐릭터 모델과 Animator Controller를 적용하여 이동, 공격, 방어 동작이 시각적으로 표현되도록 구성하였다. 또한 기존 이동 로직과 애니메이션을 함께 사용할 수 있도록 장면 구성과 제어 방식을 조정하였다.

초기 구현에서는 상태를 색상 변화로 표현하였으나, 이후에는 캐릭터 모델과 애니메이션을 활용하는 방식으로 시각적 표현을 확장하였다. 이를 통해 손 제스처와 음성 명령이 Unity 캐릭터의 이동 및 행동 반응으로 연결되는 과정을 보다 직관적으로 확인할 수 있었다.

⚙️ 저장소 구성
프로젝트는 데이터 처리 및 학습 코드, Unity 데모 프로젝트, 실행 스크립트, 환경 설정 파일 등으로 구성되어 있다. 주요 디렉터리와 파일은 다음과 같다.

hand_voice_fusion_project/
│
├── fusion_project/            # 데이터, split, 특징 추출 및 학습 코드
├── Unity/                     # Unity 데모 프로젝트
...
또는 필요에 따라 각각 별도로 실행할 수 있다.

python send_hand_cmd_latefusion.py
python send_voice_cmd_latefusion.py

3. Unity 데모 실행
Unity에서 관련 프로젝트를 열고 데모 씬을 실행한다. Python 스크립트가 UDP 명령을 전송하면 Unity 캐릭터가 해당 명령에 따라 이동 및 행동 반응을 수행한다.

✔️ 결론
본 연구를 통해 손 제스처와 음성을 결합한 멀티모달 실시간 제어 시스템이 실제 인터랙션 환경에서 활용될 가능성이 있음을 확인하였다. Early Fusion과 Late Fusion 모두 높은 이동 정확도와 안정적인 C_fusion CSR을 보였으며, 최종 실험에서는 Late Fusion이 행동 정확도와 전체 평균 성능 측면에서 소폭 높은 결과를 나타냈다.

또한 Unity 데모를 통해 인식 결과가 실제 캐릭터 이동 및 행동 표현으로 연결되는 과정을 구성함으로써, 멀티모달 HCI 시스템의 실제 적용 가능성을 확인할 수 있었다.
