...
public class CommandController : MonoBehaviour
{
    ...
    public Transform visualRoot;           // 실제 모델(팔라딘) 루트
    ...
    [Header("방향 고정 설정")]
    public bool keepFacingForward = true;  // true이면 캐릭터가 항상 앞쪽을 바라봄
    public Vector3 fixedForwardDirection = Vector3.forward; // 고정으로 바라볼 방향
    ...
    [Header("액션 반복 방지")]
    public float sameActionBlockTime = 0.8f;   // 같은 ATTACK / DEFEND 반복 실행 방지 시간
    ...
    // 같은 액션 반복 방지용 상태
    private string lastActionCmd = "";         // 마지막으로 실행한 액션 명령
    private float lastSameActionTime = -999f;  // 같은 액션이 마지막으로 실행된 시간
    ...
    void Awake()
    {
        ...
        if (!visualRoot) visualRoot = player;
        if (!animator && visualRoot) animator = visualRoot.GetComponent<Animator>();
        if (!animator && player) animator = player.GetComponentInChildren<Animator>();
    ...
    MOVE_UPDATE:
        ...
        if (!usePhysicsMove && moveDir != Vector3.zero)
        {
            ...
            // 이동 방향으로 순간 회전하지 않고, 항상 고정된 앞 방향을 바라보게 함
            LockFacingForward();

            ApplyMoveAnimationByDir(moveDir);
        ...
        else
        {
            if (!usePhysicsMove && moveDir == Vector3.zero)
            {
                ClearMoveAnimationBools();
            }
        }

        // 이동 중이 아니어도 캐릭터 방향은 계속 고정
        LockFacingForward();
    ...
    void FaceMoveDirection(Vector3 dir)
    {
        if (!visualRoot || dir == Vector3.zero) return;

        Vector3 flatDir = new Vector3(dir.x, 0f, dir.z);
        if (flatDir.sqrMagnitude < 0.0001f) return;

        visualRoot.rotation = Quaternion.LookRotation(flatDir, Vector3.up);
    }

    // 캐릭터가 항상 지정된 앞 방향을 바라보게 고정
    void LockFacingForward()
    {
        if (!keepFacingForward || !visualRoot) return;

        Vector3 flatDir = new Vector3(
            fixedForwardDirection.x,
            0f,
            fixedForwardDirection.z
        );
        ...
        visualRoot.rotation = Quaternion.LookRotation(flatDir.normalized, Vector3.up);
    }

    // 같은 ATTACK / DEFEND 명령이 UDP에 계속 남아 있어도 반복 실행되지 않도록 막는 함수
    bool CanRunAction(string cmd)
    {
        if (cmd == lastActionCmd && Time.time - lastSameActionTime < sameActionBlockTime)
        {
            return false;
        }

        lastActionCmd = cmd;
        lastSameActionTime = Time.time;
        return true;
    }
    ...
    // 현재 이동 방향에 맞는 애니메이션 Bool만 켜기
    void ApplyMoveAnimationByDir(Vector3 dir)
    {
        if (!animator) return;
