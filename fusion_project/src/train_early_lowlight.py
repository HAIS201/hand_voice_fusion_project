...
def augment_lowlight_gesture(seq, noise_std=0.01, mask_prob=0.15, max_mask_len=4):
    """
    저조도 환경용 gesture sequence 증강
    seq: (60, 63)

    1) landmark에 작은 Gaussian noise 추가
    2) 일부 연속 프레임을 0으로 마스킹
    """
    seq = seq.copy()

    # 1. landmark 좌표에 작은 잡음 추가
    noise = np.random.normal(0, noise_std, size=seq.shape).astype(np.float32)
    seq = seq + noise

    # 2. 일부 프레임 마스킹 (저조도에서 짧게 검출 불안정한 상황 모사)
    if np.random.rand() < mask_prob:
        start = np.random.randint(0, max(1, seq.shape[0] - max_mask_len))
        length = np.random.randint(1, max_mask_len + 1)
...
class EarlyFusionDataset(Dataset):
    ...
    def __init__(self, csv_file: Path, m2i: dict, a2i: dict, is_train=False):
        # split CSV 로드
    ...
    def __getitem__(self, idx):
        ...
        env = r.get("env", "")
        if self.is_train and env in ["DimQuiet", "DimFan"]:
            g = augment_lowlight_gesture(g)
...
def train(args):
    ...
    # 데이터셋 생성
    print("\n데이터셋 로드 중...")
    train_ds = EarlyFusionDataset(SPLITS_DIR / "train.csv", m2i, a2i, is_train=True)
    val_ds = EarlyFusionDataset(SPLITS_DIR / "val.csv", m2i, a2i, is_train=False)
    test_ds = EarlyFusionDataset(SPLITS_DIR / "test.csv", m2i, a2i, is_train=False)
    ...
    best_val = -1.0
    best_epoch = 0
    ckpt_path = OUT_DIR / "early_gru_lowlight_best.pth"
    ...
    # 텍스트 보고서 저장
    with open(OUT_DIR / "early_gru_lowlight_report.txt", "w", encoding="utf-8") as f:
    ...
    print(f"\n보고서가 {OUT_DIR / 'early_gru_lowlight_report.txt'} 에 저장되었습니다.")
