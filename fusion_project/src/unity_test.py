...
# Unity 쪽 UDP 주소 (다른 스크립트랑 맞춰서 사용)
...
def rel_to_npy(relpath: str):
    """data 아래 mp4 relpath → features 아래 같은 이름의 npy 경로로 매핑"""
    rel = Path(relpath)
...
class FusionTestSeqDataset(Dataset):
    ...
    테스트용 Dataset (GRU 버전):
      ...
      - 시간 축을 그대로 유지해서 반환:
          g: (T, DG)
          a: (T, DA)
      - 문자열 라벨, subset, env 같은 메타 정보도 같이 반환
    """
    ...
    def _to_seq(self, arr: np.ndarray, d_expect: int) -> np.ndarray:
        """
        다양한 형태의 npy를 (T, D_EXPECT) 형태로 통일하는 함수.
...
class GestureGRU(nn.Module):
    """손 제스처만 분류하는 GRU 브랜치 (ckpt['g_model'] 등과 함께 사용)"""
...
class VoiceGRU(nn.Module):
    """음성만 분류하는 GRU 브랜치 (ckpt['v_model'] 등과 함께 사용)"""
...
class TwoBranchLateWrapper(nn.Module):
    """
    ckpt 안에 g_model / v_model 이 따로 저장된 경우를 위해 만든 래퍼.

    - forward(g_seq, a_seq) 에서 내부적으로
      g_model(g_seq), v_model(a_seq)를 호출해서
      (logits_g, logits_v)를 반환한다. (LateFusionGRU 랑 인터페이스 동일)
    """
...
def load_late_model(gest, voic):
    ...
    케이스:
      1) ckpt["model"] 에 LateFusionGRU 전체가 있는 경우
    ...
    # case 3: "gnet" / "vnet" 쌍 (지금 쓰고 있는 형태)
    ...
    # case 4: 위 세 가지 어디에도 해당 안 되면 에러
    else:
        raise KeyError(
            "late_best.pth 안에 'model' / 'g_model'+'v_model' / 'gnet'+'vnet' "
            f"형태가 없음. 실제 keys={list(ckpt.keys())}"
        )
...
def main(args):
    ...
    print(f"[INFO] mode={args.mode}, only_cfusion={args.only_cfusion}, delay={args.delay}s")

    idx = 0
    for batch in loader:
        subset = batch["subset"][0]
        # --only_cfusion 옵션이 켜져 있으면 C_fusion 만 재생
        if args.only_cfusion and subset != "C_fusion":
            continue
        ...
        # DataLoader 에서 이미 batch 축이 붙어 있기 때문에 (B,T,D) 형태
        g_seq = batch["g"].to(DEVICE).float()
        a_seq = batch["a"].to(DEVICE).float()
        ...
        # GT 가 'NONE' 인 모달은 Unity 쪽으로 안 보내서 혼동 줄이기
        g_cmd = g_pred if g_gt != "NONE" else "NONE"
        v_cmd = v_pred if v_gt != "NONE" else "NONE"
        send_to_unity(sock, g_cmd, v_cmd)

        time.sleep(args.delay)

    print("\n[DONE] test 셋 재생 끝났습니다. early / late 모드 바꿔서 한 번 더 돌려봐도 좋습니다.")
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--only_cfusion",
        action="store_true",
        help="C_fusion subset만 재생 (기본값: False, 전체 test 사용)",
    )
    ap.add_argument(
        "--delay",
        type=float,
